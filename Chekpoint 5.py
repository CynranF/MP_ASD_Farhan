...
class WebtoonLinkedList:
    ...
    def quick_sort_by_name(self, head, ascending=True):
        current = head
        webtoon_list = []
        while current:
            webtoon_list.append(current)
            current = current.next

        webtoon_list.sort(key=lambda x: x.judul.lower(), reverse=not ascending)

        self.head = webtoon_list[0] if webtoon_list else None
        for i in range(len(webtoon_list) - 1):
            webtoon_list[i].next = webtoon_list[i+1]
        if webtoon_list:
            webtoon_list[-1].next = None

    def quick_sort_by_rating(self, head, ascending=True):
        current = head
        webtoon_list = []
        while current:
            webtoon_list.append(current)
        ...
        webtoon_list.sort(key=lambda x: x.rating, reverse=not ascending)
...
def main():
    ...
    while True:
        ...
        if pilihan == '1':
            ...
            print("| [6] Kembali                             |")
            print("==========================================")
            pilihan = input("Masukkan pilihan lihat webtoon (1-6): ")


            if pilihan == '1':
                webtoon_list.tampilkan_daftar_webtoon()
            elif pilihan == '2':
                webtoon_list.quick_sort_by_name(webtoon_list.head, ascending=True)
                print("Daftar Webtoon setelah sorting by Judul (Ascending):")
                webtoon_list.tampilkan_daftar_webtoon()
            elif pilihan == '3':
                webtoon_list.quick_sort_by_name(webtoon_list.head, ascending=False)
                print("Daftar Webtoon setelah sorting by Judul (Descending):")
                webtoon_list.tampilkan_daftar_webtoon()
            elif pilihan == '4':
                webtoon_list.quick_sort_by_rating(webtoon_list.head, ascending=True)
                print("Daftar Webtoon setelah sorting by Rating (Ascending):")
                webtoon_list.tampilkan_daftar_webtoon()
            elif pilihan == '5':
                webtoon_list.quick_sort_by_rating(webtoon_list.head, ascending=False)
                print("Daftar Webtoon setelah sorting by Rating (Descending):")
                webtoon_list.tampilkan_daftar_webtoon()
            elif pilihan == '6':
